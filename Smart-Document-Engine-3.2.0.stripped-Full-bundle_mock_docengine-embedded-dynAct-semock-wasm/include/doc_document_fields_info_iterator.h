...
/// Forward declaration for the internal implementation of
///     the DocDocumentTableFieldColumnsInfoInterator class
class DocDocumentTableFieldColumnsInfoInteratorImpl;
...
/**
 * @brief Const-ref iterator for a collection of columns inside document table field
 */
class SE_DLL_EXPORT DocDocumentTableFieldColumnsInfoInterator {

private:
  /// Private ctor from internal implementation
  DocDocumentTableFieldColumnsInfoInterator(const DocDocumentTableFieldColumnsInfoInteratorImpl& pimpl);

public:
  /// Copy ctor
  DocDocumentTableFieldColumnsInfoInterator(const DocDocumentTableFieldColumnsInfoInterator& other);
  /// Assignment operator
  DocDocumentTableFieldColumnsInfoInterator& operator =(const DocDocumentTableFieldColumnsInfoInterator& other);
  /// Non-trivial dtor
  ~DocDocumentTableFieldColumnsInfoInterator();

  /// Factory method - constructs an iterator from its internal implementation
  static DocDocumentTableFieldColumnsInfoInterator ConstructFromImpl(
    const DocDocumentTableFieldColumnsInfoInteratorImpl& pimpl);
  ...
  bool Equals(const DocDocumentTableFieldColumnsInfoInterator& rvalue) const;
  /// Returns true iff this instance and rvalue point to the same column
  bool operator ==(const DocDocumentTableFieldColumnsInfoInterator& rvalue) const;
  /// Returns true iff this instance and rvalue point to the different column
  bool operator !=(const DocDocumentTableFieldColumnsInfoInterator& rvalue) const;

private:
  /// Pointer to internal implementation
  class DocDocumentTableFieldColumnsInfoInteratorImpl* pimpl_;
};
