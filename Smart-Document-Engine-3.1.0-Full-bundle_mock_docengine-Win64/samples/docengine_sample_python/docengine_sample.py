      ...
      if doc.GetAttribute('type') != "":
        if doc.GetAttribute('type') != "untypified":
          ...
          while(f_it != doc.TextFieldsEnd()):
            SortedTextFields.append(f_it.GetField())
            f_it.Advance()
          if (len(SortedTextFields) > 0 and SortedTextFields[0].GetBaseFieldInfo().HasAttribute("order")):
            SortedTextFields.sort(key=lambda x: int(x.GetBaseFieldInfo().GetAttribute("order")))
          print('    Text fields ({} in total):'.format( doc.GetTextFieldsCount()))
          for TextField in SortedTextFields:
            print('        {0} : {1}'.format( TextField.GetBaseFieldInfo().GetName(),
                  TextField.GetOcrString().GetFirstString().GetCStr()))
            print("            Connected objects:\n")
            c_it = TextField.GetBaseFieldInfo().ConnectedTextObjectsBegin(phys_doc)
            while(c_it != TextField.GetBaseFieldInfo().ConnectedTextObjectsEnd(phys_doc)):
