...
public class DocEngineSample {
    ...
    if (recog_result.GetDocumentsCount() > 0) {
      for (DocumentsIterator doc_it = recog_result.DocumentsBegin();
        ...
        if (!doc.GetAttribute("type").isEmpty()) { 
          if (!Objects.equals(doc.GetAttribute("type"), "untypified")) {
            ...
            List<DocTextField> SortedTextFields = new ArrayList<>();
            for (DocTextFieldsIterator f_it = doc.TextFieldsBegin();
                !f_it.Equals(doc.TextFieldsEnd());
                f_it.Advance()) {
              SortedTextFields.add(f_it.GetField());
            }
            if (SortedTextFields.size() > 0 && SortedTextFields.get(0).GetBaseFieldInfo().HasAttribute("order")) {
              SortedTextFields.sort((a, b) -> {
                return Integer.compare(Integer.parseInt(a.GetBaseFieldInfo().GetAttribute("order")), Integer.parseInt(b.GetBaseFieldInfo().GetAttribute("order")));
              });
            }
            System.out.printf("    Text fields (%d in total):\n", doc.GetTextFieldsCount());
            for (DocTextField TextField : SortedTextFields) {
              System.out.printf("        %-25s : %s\n", TextField.GetBaseFieldInfo().GetName(),
                    TextField.GetOcrString().GetFirstString().GetCStr());
              System.out.printf("            Connected objects:\n");
              for (DocTextObjectsCrossPageIterator c_it = TextField.GetBaseFieldInfo().ConnectedTextObjectsBegin(phys_doc);
                ...
                for (int idx = 0; idx < conn_geometry.GetPointsCount(); idx++) {
                  System.out.printf(" (%f, %f) ", conn_geometry.GetPoint(idx).getX(), conn_geometry.GetPoint(idx).getY());
                }
                System.out.printf("]\n");
              }
            }
  ...
  public static void main(String[] args) {
    ...
    if (args.length != 2 && args.length != 3) {
      System.out.printf("Version %s. Usage: DocEngineSample " +
