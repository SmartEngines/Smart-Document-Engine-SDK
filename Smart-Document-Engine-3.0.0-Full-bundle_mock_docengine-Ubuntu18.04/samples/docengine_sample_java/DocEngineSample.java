        ...
        if (!doc.GetAttribute("type").isEmpty()) { 
          if (!Objects.equals(doc.GetAttribute("type"), "untypified")) {
            ...
            for (DocTextFieldsIterator f_it = doc.TextFieldsBegin();
              ...
              for (DocTextObjectsCrossPageIterator c_it = f_it.GetField().GetBaseFieldInfo().ConnectedTextObjectsBegin(phys_doc);
                        !c_it.Equals(f_it.GetField().GetBaseFieldInfo().ConnectedTextObjectsEnd(phys_doc));
                        c_it.Advance()) {
