...
class SampleViewController: UIViewController,
  ...
  func setMessage(_ message: String) {
    ...
    self.resultTextFields.append((message, ""))
  }
  ...
  func setResult(result: SEDocResultRef) {
    ...
    let currentMode = engineInstance.sessionSettings!.getCurrentMode()
    if currentMode == "crop" || currentMode == "crop_advanced" {
      var templateExtracted : Bool = false
      let graphical = result.getGraphicalStructure()
      let collIter = graphical.objectsCollectionsBegin()
      let collEnd = graphical.objectsCollectionsEnd()
      while !collIter.isEqual(toIter: collEnd) {
        if collIter.getObjectsCollection().objectType() == "template" {
          if collIter.getObjectsCollection().getObjectsCount() > 0 {
            let ser_params = SECommonSerializationParameters()
            let serializer = SECommonSerializer(jsonSerializerWithParams: ser_params)
            collIter.getObjectsCollection().basicObjectsBegin().getBasicObject().getBaseObjectInfo().getGeometry().serialize(serializer)
            resultTextFields.append(("template", serializer.getString()))
    ...
    let doc = result.documentsBegin().getDocument()
  ...
  override func viewDidLoad() {
    ...
    DispatchQueue.main.async {
      ...
      let bundlePaths = Bundle.main.paths(forResourcesOfType: "se", inDirectory: "data-zip")
  ...
  func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : Any]) {
    ...
    picker.dismiss(animated: true, completion: {
      DispatchQueue.main.async { [weak self] in
        ...
        self?.engineInstance.processSingleImage(from: image)
